...
export default class PddlExecutor {
     ...
     * @param {PddlPlan} plan 
     */
    async exec (plan, no_print = false) {
        ...
        for (const step of plan) {
            if (step.parallel) {
                if(!no_print) console.log( 'Starting concurrent step', step.action, ...step.args )
            }
            else {
                await Promise.all(previousStepGoals)
                previousStepGoals = []
                if(!no_print) console.log( 'Starting sequential step', step.action, ...step.args )
            }
