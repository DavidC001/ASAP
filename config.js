...
const DASHBOARD = false; // if true, the dashboard will be available at http://localhost:PORT
...
const REPLAN_MOVE_INTERVAL = (USE_PDDL) ? Math.Infinity : 2; // Number of moves before hard replanning
const SOFT_REPLAN_INTERVAL = (USE_PDDL) ? 2 : Math.Infinity; // Number of moves before soft replanning
const INTENTION_REVISION_INTERVAL = 100; // Interval between intention revision
const BASE_PLANNING_TIME = 0; // Time taken by the planner to plan the actions
const PLANNING_TIME_DECAY = 0.9; // Decay rate for the planning time
const STOP_WHILE_PLANNING_INTERVAL = 1000; // Interval used to check if another intention wins before planning finishes
const PENALITY_RATE_CARRIED_PARCELS = 1.5; // Rate of penalty for each carried parcel
const BASE_MOVE_SLACK = 100; // Slack time to consider in the movement duration
const SLACK_DECAY = 0.9; // Decay rate for the slack time

// Planner hyperparameters
const MAX_WAIT = 1; // Maximum number of moves to wait on a tile
const MAX_EXPLORE_PATH_LENGTH = 20; // Maximum length of the path to explore
const PROBABILITY_KEEP_BEST_TILE = 0.75; // Probability to keep the tile when ther is a tie in the exploration
...
const MAX_FUTURE = 2; // Maximum number of future to calculate
...
export {
    ...
    MAX_RETRIES, REPLAN_MOVE_INTERVAL, SOFT_REPLAN_INTERVAL, USE_PDDL, 
